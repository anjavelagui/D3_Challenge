// function used for updating x-scale and y-scaleupon click on axis label
function xScale(csvData, xLabel) {
  ...
  //function to updating circlesGroup for X and Y axis
  function renderXCircles(circlesGroup, newXScale, xLabel) {
    circlesGroup.transition().duration(1000).attr("cx", d => newXScale(d[xLabel]));
    return circlesGroup;
  }
  
  function renderYCircles(circlesGroup, newYScale, yLabel) {
    circlesGroup.transition().duration(1000).attr("cy", d => newYScale(d[yLabel]));
    return circlesGroup;
  }
  
  // Updating tooltips for circles X and Y axis
  function renderXText(circlesGroup, newXScale, yLabel) {
    circlesGroup.transition().duration(1000).attr("dx", d => newXScale(d[yLabel]));
    return circlesGroup;
  }
  
  function renderYText(circlesGroup, newYScale, yLabel) {
    circlesGroup.transition().duration(1000).attr("dy", d => newYScale(d[yLabel])+5);
    return circlesGroup;
  }
  // Setting and creating regression 
  ...
  function renderRegression(csvData, plotRegress, newXScale, newYScale, xLabel, yLabel, xArr) {
    ...
    plotRegress.transition().duration(1000).attr("d", newLine(newRegressPoints));
    return plotRegress;
  }
  
  // Formating to US currency
